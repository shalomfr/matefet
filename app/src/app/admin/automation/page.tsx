  ...
  return (
    <>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <StatCard icon={Zap} label="אוטומציות פעילות" value={String(activeCount)} color="#4a7cff" />
      ...
      <div className="card-dark p-5 mb-6 border-r-4 border-r-[#4a7cff] flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-[--color-text] mb-1">בנה אוטומציה חדשה</h3>
          <p className="text-sm text-[--color-muted]">הגדר טריגר, תנאים ופעולות - והכל רץ לבד</p>
        </div>
      ...
      <div className="card-dark p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-bold text-[--color-text]">אוטומציות ({workflows.length})</h3>
          <div className="flex gap-2">
            {["הכל", "פעילות", "מושהות"].map((tab, i) => (
              <button
                key={tab}
                className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                  i === 0 ? "bg-[#4a7cff]/10 text-[#4a7cff]" : "text-[--color-muted] hover:bg-white/5"
                }`}
              >
                {tab}
              </button>
        ...
        <div className="space-y-3">
          {workflows.map((wf) => (
            <div
              key={wf.name}
              className="p-4 rounded-xl bg-[--color-surface2] border border-[--color-border] hover:border-[#4a7cff]/30 transition-all"
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
                    wf.status === "active" ? "bg-[#4a7cff]/10" : "bg-[--color-border]/30"
                  }`}>
                    <Zap size={17} className={wf.status === "active" ? "text-[#4a7cff]" : "text-[--color-muted]"} />
                  </div>
                  <div>
                    <div className="text-sm font-bold text-[--color-text]">{wf.name}</div>
                    <div className="text-xs text-[--color-muted]">טריגר: {wf.trigger}</div>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <span className="badge badge-purple">{wf.runs} הרצות</span>
                  <span className="text-xs text-[--color-muted]">{wf.lastRun}</span>
                  <button className="text-[--color-muted] hover:text-[#4a7cff]">
                    {wf.status === "active" ? <ToggleRight size={22} className="text-[#4a7cff]" /> : <ToggleLeft size={22} />}
                  </button>
              ...
              <div className="flex items-center gap-2 mr-12 text-xs text-[--color-muted]">
                <span className="badge badge-warning">טריגר</span>
                <ArrowLeft size={12} className="text-[--color-muted]" />
                {wf.actions.split("→").map((action, i) => (
                  <span key={i} className="flex items-center gap-2">
                    <span className="badge badge-success">{action.trim()}</span>
                    {i < wf.actions.split("→").length - 1 && <ArrowLeft size={12} className="text-[--color-muted]" />}
                  </span>
                ))}
              </div>
            </div>
      ...
      <div className="card-dark p-5">
        <h3 className="text-base font-bold text-[--color-text] mb-4">הרצות אחרונות</h3>
          ...
          {recentExecutions.map((exec, i) => (
            <div key={i} className="flex items-center justify-between p-3 rounded-xl hover:bg-white/[0.03] transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-medium text-[--color-text]">{exec.workflow}</div>
                  <div className="text-xs text-[--color-muted]">{exec.trigger}</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-[--color-muted]">{exec.time}</span>
              </div>
            </div>
          ))}
        </div>
